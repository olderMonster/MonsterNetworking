...
extension APIManager {
    ...
    func afterGlobalInterceptorSucceedIn(result: SResult<HTTPReponse, APIError>) ->  SResult<HTTPReponse, APIError>? {
        guard let interceptor = self.interceptor else {
            return nil
        }
        if case .success(let response) = result {
            let childResult = interceptor.succeedInRequest(response: response)
            if  case .success(let res) = childResult {
                return SResult.success(res)
            }else if case .failure(let err) = childResult {
